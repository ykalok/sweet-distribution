  ...
  const fetchOrders = async () => {
    try {
      ...
      for (const order of orderList) {
        if (order.status !== 'PENDING') { fetchPaymentInfo(order.id); fetchInvoiceInfo(order.id); }
      }
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
      ...
      <div className="space-y-4">
        {orders.map((order, idx) => {
          ...
          return (
            <div key={order.id} className="card overflow-hidden animate-slide-up" style={{ animationDelay: `${idx * 0.05}s` }}>
              {/* Header */}
              <div className="p-5 cursor-pointer" onClick={() => setExpandedOrder(isExpanded ? null : order.id)}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${cfg.bg} border ${cfg.color}`}>
                      {cfg.icon}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`badge border ${cfg.bg} ${cfg.color}`}>{order.status}</span>
              ...
              {isExpanded && (
                <div className="border-t border-gray-100 p-5 animate-fade-in">
                  ...
                  <div className="p-3 bg-gray-50 rounded-xl text-sm">
                    ...
                    {order.notes && (
                      <div className="mt-1"><span className="font-medium text-gray-700">Notes: </span><span className="text-gray-500">{order.notes}</span></div>
                    )}
                  </div>
                </div>
              )}
